...
import { Package, CalendarCheck, CheckCircle2, Clock, PlusCircle } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
        ...
        <div className="divide-y divide-slate-800">
          {[
            { id: 1, title: 'Fresh Veg Thali Meals', qty: '40 Servings', status: 'RESERVED', time: '10 mins ago' },
            { id: 2, title: 'Paneer Butter Masala & Roti', qty: '25 Servings', status: 'AVAILABLE', time: '1 hour ago' },
            { id: 3, title: 'Mixed Fruit Salad Containers', qty: '15 Servings', status: 'COMPLETED', time: '3 hours ago' },
          ].map((item) => (
            <div key={item.id} className="py-3 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-100">{item.title}</p>
                <p className="text-xs text-slate-400">{item.qty} • Posted {item.time}</p>
              </div>
              <span
